...
/*
  ...
  Even more exciting, we'll start using the Arduino's serial
  port to send data back to your main computer! Up until now,
  we've been limited to using simple LEDs for output. We'll
  see that the Arduino can also easily output all kinds of
  text and data.
...
void loop()
{
  ...
  // We'll declare four floating-point variables
  ...
  // turn off all LEDs
  if (degreesC < baselineTemp + 2) {
    digitalWrite(RED_PIN, LOW);
    digitalWrite(GREEN_PIN, LOW);
    digitalWrite(BLUE_PIN, LOW);
  } // if the temperature rises 2-4 degrees, turn an LED on
  else if (degreesC >= baselineTemp + 2 && degreesC < baselineTemp + 4) {
    digitalWrite(RED_PIN, HIGH);
    digitalWrite(GREEN_PIN, LOW);
    digitalWrite(BLUE_PIN, LOW);
  } // if the temperature rises 4-6 degrees, turn a second LED on
  else if (degreesC >= baselineTemp + 4 && degreesC < baselineTemp + 6) {
    digitalWrite(RED_PIN, HIGH);
    digitalWrite(GREEN_PIN, HIGH);
    digitalWrite(BLUE_PIN, LOW);
  } // if the temperature rises more than 6 degrees, turn all LEDs on
  else if (degreesC >= baselineTemp + 6) {
    digitalWrite(RED_PIN, HIGH);
    digitalWrite(GREEN_PIN, HIGH);
    digitalWrite(BLUE_PIN, HIGH);
  }
  
  delay(10); // repeat once per 10ms (change as you wish!)
}
